...
type StorageExplorerCommandParams = {
  ...
  openFile?: (item: ExplorerItem, fallback: () => void) => void;
  ...
  return [
    {
      ...
      run: (runtime) => {
        ...
        const fallback = () => runtime.fileViewer.openFile(item.id);
        if (openFile) openFile(item, fallback);
        else fallback();
      },
    },
    {
    ...
    },
    {
      ...
      isVisible: (runtime) => runtime.targetItems.length > 0,
      isDisabled: (runtime) =>
        storage.isCreatingArchive || runtime.targetItems.length === 0,
      run: (runtime) => {
        void storage.createArchiveItems(runtime.targetItems);
      },
    },
    {
      id: "details",
